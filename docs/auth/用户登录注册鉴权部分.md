...
await api(`/api/auth/devices/${otherDevice.device_id}`, { method: 'DELETE', token });
// ✓ 当前 token 仍然有效

// 场景 2: 删除当前设备
const currentDevice = devices.devices.find(d => d.is_current);
await api(`/api/auth/devices/${currentDevice.device_id}`, { method: 'DELETE', token });
