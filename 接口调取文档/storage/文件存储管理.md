...
**🚀 预签名URL**：客户端直连MinIO，支持视频流式播放、Range请求、断点续传。
...
> **好友文件自动消息**：当 `storage_location` 为 `friend_messages` 时，上传完成后自动在聊天记录中插入文件消息。
> - 消息类型：根据文件类型自动判断（`image`/`video`/`file`）
> - 消息内容：`[图片] 文件名`、`[视频] 文件名`、`[文件] 文件名`
> - 返回字段：`message_uuid` 和 `message_send_time` 可用于前端更新聊天界面
...
1. **请求上传**：与个人文件相同，但指定 `storage_location: "friend_messages"`
2. **验证好友关系**：后端自动验证
3. **上传文件**：与个人文件相同
4. **自动授权**：上传完成后自动授权好友访问

### 请求示例

```javascript
// 好友文件上传
async function uploadFriendFile(file, friendId) {
  const accessToken = localStorage.getItem('access_token');
  ...
  const response = await fetch('http://localhost:8080/api/storage/upload/request', {
    ...
    body: JSON.stringify({
      file_type: 'friend_image',           // friend_image/friend_video/friend_document
  ...
  return {
    file_uuid: result.file_url.split('/').pop(),
...
```javascript
// 发送图片消息
async function sendImageMessage(friendId, file) {
  const accessToken = localStorage.getItem('access_token');
  
  // 1. 上传文件
  const { file_uuid, file_url } = await uploadFriendFile(file, friendId);
  ...
  const response = await fetch('http://localhost:8080/api/messages', {
    ...
    body: JSON.stringify({
      ...
      file_uuid: file_uuid  // 使用 file_uuid
    })
  });
