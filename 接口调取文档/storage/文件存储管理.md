...
- **URL**: `/api/storage/file/{uuid}/presigned-url`
- **方法**: `POST`
- **需要鉴权**: 是（Bearer Token）
...
async function getPresignedUrl(uuid, filename, retryCount = 0) {
  ...
  if (isLargeFile) {
    ...
    endpoint = `http://localhost:8080/api/storage/file/${uuid}/presigned-url/extended`;
    body = { operation: 'download', estimated_download_time: estimatedSeconds };
  } else {
    // 普通文件：3小时有效期
    endpoint = `http://localhost:8080/api/storage/file/${uuid}/presigned-url`;
...
async function getPresignedUrl(uuid, filename) {
  ...
  // 2. 请求新的预签名URL
  const response = await fetch(
    `http://localhost:8080/api/storage/file/${uuid}/presigned-url`,
    {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${accessToken}`,
...
```bash
# 获取预签名URL
curl -X POST "http://localhost:8080/api/storage/file/cb31fb54-770d-4198-84d6-18ce9c6d290f/presigned-url" \
...
- **URL**: `/api/storage/file/{uuid}/presigned-url/extended`
- **方法**: `POST`
- **需要鉴权**: 是（Bearer Token）
...
async function getLargeFilePresignedUrl(uuid, filename, fileSize) {
  ...
  if (fileSize >= LARGE_FILE_THRESHOLD) {
    ...
    const response = await fetch(
      `http://localhost:8080/api/storage/file/${uuid}/presigned-url/extended`,
      {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${accessToken}`,
