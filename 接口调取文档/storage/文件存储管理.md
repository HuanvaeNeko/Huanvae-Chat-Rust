...
统一文件存储管理接口，支持文件上传、下载、预览功能。**所有文件上传均使用预签名URL直传MinIO**，后端不参与数据传输。

**✨ UUID映射机制**：采用UUID映射+权限表实现跨用户文件去重，相同文件只存储一份物理副本。

**⚡ 采样哈希**：小文件完整哈希，大文件采样哈希（元信息+开头/中间/结尾各10MB），避免浏览器内存溢出。

**🚀 预签名URL直传**：客户端直连MinIO上传和下载，支持视频流式播放、Range请求、断点续传。

## 核心功能

- ✅ **UUID映射去重**（秒传，跨用户生效）
- ✅ **采样哈希计算**（避免大文件内存溢出）
- ✅ **预签名PUT直传**（< 5GB，单次PUT上传）
- ✅ **预签名分片上传**（≥ 5GB，分片上传）
...
```json
{
  ...
  "presigned_url": "http://localhost/user-file/xxx?X-Amz-Algorithm=AWS4-HMAC-SHA256&...",
  "expires_in": 3600,
...
文件上传采用**预签名URL直传MinIO**方式，后端不参与数据传输：

| 文件大小 | 上传模式 | 说明 |
|---------|---------|------|
| < 5GB | `presigned_put` | 单次PUT请求直传MinIO |
| ≥ 5GB | `presigned_multipart` | 分片上传到MinIO |

### 预签名PUT上传流程

1. 调用 `request` 接口获取 `presigned_url`
2. 使用 `PUT` 方法直传文件到MinIO
3. 调用 `confirm` 接口确认上传完成

### Fetch API 调用示例

```javascript
// 使用预签名URL直传文件到MinIO
async function uploadWithPresignedUrl(file, presignedUrl) {
  ...
  console.log('文件已成功上传到MinIO');
  return true;
}

// 带进度的上传（使用XMLHttpRequest）
...
async function uploadFileComplete(file, onProgress) {
  try {
    ...
    // 4. 根据模式上传
    if (uploadInfo.mode === 'presigned_put') {
      // <5GB: 预签名PUT直传
      console.log('使用预签名PUT直传...');
      ...
      return {
        ...
        messageUuid: confirmResult.message_uuid // 好友/群聊文件会有此字段
      };
      
    } else if (uploadInfo.mode === 'presigned_multipart') {
      // ≥5GB: 分片上传
      console.log('使用分片上传...');
      await uploadLargeFile(file, uploadInfo, onProgress);
      
      // 分片上传的confirm在complete_multipart接口中处理
      console.log('分片上传完成！');
      return {
        success: true,
        instant: false
...
fileInput.addEventListener('change', async (e) => {
  ...
  try {
    const result = await uploadFileComplete(file, (percent) => {
...
| 视频 | < 5GB | 预签名PUT | 在线预览 |
| 文档 | < 5GB | 预签名PUT | 仅下载 |
| 超大文件 | 5GB - 30GB | 预签名分片 | 仅下载 |

**上传模式说明**：
- **presigned_put**：单次PUT请求，文件直传MinIO，适用于 < 5GB 文件
- **presigned_multipart**：分片上传，每片50MB，适用于 ≥ 5GB 文件
...
2. **预签名URL**：有效期默认1小时（<5GB）或24小时（≥5GB），过期需重新请求
3. **confirm确认**：预签名上传完成后**必须调用confirm接口**，否则文件状态为`pending_confirm`
4. **直传MinIO**：所有上传都直连MinIO，后端不参与数据传输
5. **秒传功能**：相同哈希的文件会自动秒传，节省时间和带宽
6. **进度跟踪**：使用XMLHttpRequest的upload.onprogress事件跟踪上传进度
7. **WebSocket通知**：好友/群聊文件在confirm后会触发WebSocket实时通知
