# Group Messages 群消息接口（Fetch API）

群消息相关接口的前端调用示例，使用 Fetch API。

## 🔧 通用工具函数

```js
const BASE = 'http://localhost:8080';

/**
 * 通用 API 请求封装
 */
async function api(path, { method='GET', token, body }={}) {
  const headers = {};
  
  if (body) {
    headers['Content-Type'] = 'application/json';
  }
  
  if (token) {
    headers['Authorization'] = `Bearer ${token}`;
  }
  
  const res = await fetch(`${BASE}${path}`, {
    method,
    headers,
    body: body ? JSON.stringify(body) : undefined
  });
  
  const data = await res.json().catch(() => ({}));
  
  if (!res.ok) {
    throw new Error(`${res.status} ${JSON.stringify(data)}`);
  }
  
  return data;
}
```

---

## 📋 接口列表

### 1. 发送群消息

**端点**：`POST /api/group-messages`

**功能**：向群聊发送消息

**限制**：
- 必须是群成员
- 不能被禁言

**请求体**：
```typescript
{
  group_id: string,          // 群聊ID（必填）
  message_content: string,   // 消息内容（必填）
  message_type: string,      // 消息类型（必填）
  file_uuid?: string,        // 文件UUID（可选，发送文件时使用）
  file_url?: string,         // 文件URL（可选）
  file_size?: number,        // 文件大小（可选）
  reply_to?: string          // 回复的消息UUID（可选）
}
```

**消息类型**：
| 类型 | 说明 |
|------|------|
| `text` | 文本消息 |
| `image` | 图片消息 |
| `video` | 视频消息 |
| `file` | 文件消息 |
| `system` | 系统消息 |

**请求示例**：

```js
// 发送文本消息
const result = await api('/api/group-messages', {
  method: 'POST',
  token,
  body: {
    group_id: '019ae4ec-0dfe-7ac1-966e-876e9755561c',
    message_content: '大家好！',
    message_type: 'text'
  }
});

console.log(result);
// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "message_uuid": "019ae4ef-cbf6-7171-88c5-6a891004fc8f",
//     "send_time": "2025-12-03T15:58:31.926019Z"
//   }
// }
```

```js
// 发送图片消息
const result = await api('/api/group-messages', {
  method: 'POST',
  token,
  body: {
    group_id: '019ae4ec-0dfe-7ac1-966e-876e9755561c',
    message_content: '[图片]',
    message_type: 'image',
    file_uuid: 'd2f612d5-70b0-4d4e-8779-86cf6aeb2b30'
  }
});
```

```js
// 回复消息
const result = await api('/api/group-messages', {
  method: 'POST',
  token,
  body: {
    group_id: '019ae4ec-0dfe-7ac1-966e-876e9755561c',
    message_content: '我同意你的观点',
    message_type: 'text',
    reply_to: '019ae4ef-cbf6-7171-88c5-6a891004fc8f'
  }
});
```

**错误响应**：

被禁言（403）：
```json
{
  "success": false,
  "code": 403,
  "error": "你已被禁言，无法发送消息"
}
```

非群成员（403）：
```json
{
  "success": false,
  "code": 403,
  "error": "你不是该群成员"
}
```

---

### 2. 获取群消息列表

**端点**：`GET /api/group-messages?group_id=xxx&before_time=xxx&limit=50`

**参数**：
| 参数 | 类型 | 必填 | 说明 |
|------|------|-----|------|
| `group_id` | UUID | ✅ | 群聊ID |
| `before_time` | string | ❌ | 时间戳分页，ISO 8601 格式 |
| `limit` | number | ❌ | 返回条数，默认 50，最大 500 |

> 💡 **性能优化**:
> - 使用时间戳分页，避免子查询
> - 使用 JOIN 一次性获取消息和发送者信息，消除 N+1 查询问题
> - 性能提升约 50-80%

**请求示例**：
```js
// 获取最新消息
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
const messages = await api(`/api/group-messages?group_id=${groupId}&limit=50`, { token });

console.log(messages);
// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "messages": [
//       {
//         "message_uuid": "019ae4ef-cbf6-7171-88c5-6a891004fc8f",
//         "group_id": "019ae4ec-0dfe-7ac1-966e-876e9755561c",
//         "sender_id": "user_a",
//         "sender_nickname": "用户A",
//         "sender_avatar_url": "",
//         "message_content": "大家好！",
//         "message_type": "text",
//         "file_uuid": null,
//         "file_url": null,
//         "file_size": null,
//         "reply_to": null,
//         "send_time": "2025-12-03T15:58:31.926019Z",
//         "is_recalled": false
//       }
//     ],
//     "has_more": false
//   }
// }
```

```js
// 使用时间戳分页查询
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
const lastMessage = messages.data.messages[messages.data.messages.length - 1];
const beforeTime = encodeURIComponent(lastMessage.send_time);
const olderMessages = await api(
  `/api/group-messages?group_id=${groupId}&before_time=${beforeTime}&limit=50`,
  { token }
);
```

**响应字段说明**：
| 字段 | 类型 | 说明 |
|------|------|------|
| `message_uuid` | UUID | 消息唯一标识 |
| `group_id` | UUID | 群聊ID |
| `sender_id` | string | 发送者ID |
| `sender_nickname` | string | 发送者昵称 |
| `sender_avatar_url` | string | 发送者头像URL |
| `message_content` | string | 消息内容（已撤回显示 "[消息已撤回]"） |
| `message_type` | string | 消息类型 |
| `file_uuid` | string \| null | 文件UUID |
| `file_url` | string \| null | 文件URL |
| `file_size` | number \| null | 文件大小（字节） |
| `reply_to` | UUID \| null | 回复的消息UUID |
| `send_time` | string | 发送时间（ISO 8601） |
| `is_recalled` | boolean | 是否已撤回 |
| `has_more` | boolean | 是否有更多历史消息 |

---

### 3. 删除消息（个人）

**端点**：`DELETE /api/group-messages/delete`

**功能**：删除消息（仅对自己不可见，不影响其他群成员）

**请求体**：
```typescript
{
  message_uuid: string  // 要删除的消息UUID
}
```

**请求示例**：
```js
await api('/api/group-messages/delete', {
  method: 'DELETE',
  token,
  body: {
    message_uuid: '019ae4ef-cbf6-7171-88c5-6a891004fc8f'
  }
});

// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "success": true,
//     "message": "消息已删除"
//   }
// }
```

**说明**：
- 这是"软删除"，仅对当前用户不可见
- 其他群成员仍然可以看到这条消息
- 无法撤销删除操作

---

### 4. 撤回消息

**端点**：`POST /api/group-messages/recall`

**功能**：撤回消息（所有人都看不到）

**权限**：
- **发送者**：只能撤回 **2 分钟内** 发送的消息
- **群主/管理员**：可以撤回任意消息

**请求体**：
```typescript
{
  message_uuid: string  // 要撤回的消息UUID
}
```

**请求示例**：
```js
await api('/api/group-messages/recall', {
  method: 'POST',
  token,
  body: {
    message_uuid: '019ae4ef-cbf6-7171-88c5-6a891004fc8f'
  }
});

// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "success": true,
//     "message": "消息已撤回"
//   }
// }
```

**撤回后效果**：
- 所有人看到的消息内容变为 `[消息已撤回]`
- 文件信息不再返回
- 消息的 `is_recalled` 字段变为 `true`

**错误响应**：

超过撤回时限（400）：
```json
{
  "success": false,
  "code": 400,
  "error": "只能撤回 2 分钟内发送的消息"
}
```

权限不足（403）：
```json
{
  "success": false,
  "code": 403,
  "error": "只有群主或管理员可以撤回他人的消息"
}
```

---

## 🎨 完整示例：群聊消息页面

```html
<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
  <title>群聊消息</title>
  <style>
    .chat-container { max-width: 800px; margin: 0 auto; padding: 20px; }
    .messages { height: 500px; overflow-y: auto; border: 1px solid #ddd; padding: 10px; }
    .message { margin-bottom: 15px; padding: 10px; background: #f5f5f5; border-radius: 8px; }
    .message.recalled { opacity: 0.6; font-style: italic; }
    .message-header { display: flex; align-items: center; margin-bottom: 5px; }
    .avatar { width: 32px; height: 32px; border-radius: 50%; margin-right: 8px; }
    .sender-name { font-weight: bold; margin-right: 10px; }
    .send-time { color: #999; font-size: 12px; }
    .message-content { margin-left: 40px; }
    .input-area { display: flex; margin-top: 10px; }
    .input-area input { flex: 1; padding: 10px; }
    .input-area button { padding: 10px 20px; background: #007bff; color: white; border: none; }
  </style>
</head>
<body>
  <div class="chat-container">
    <h1>群聊消息</h1>
    
    <div class="messages" id="messagesContainer">
      <!-- 消息列表 -->
    </div>
    
    <div class="input-area">
      <input type="text" id="messageInput" placeholder="输入消息...">
      <button onclick="sendMessage()">发送</button>
    </div>
  </div>

  <script>
    const BASE = 'http://localhost:8080';
    const token = localStorage.getItem('access_token');
    const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c'; // 从URL或状态获取
    
    // API 请求封装
    async function api(path, { method='GET', token, body }={}) {
      const headers = {};
      if (body) headers['Content-Type'] = 'application/json';
      if (token) headers['Authorization'] = `Bearer ${token}`;
      
      const res = await fetch(`${BASE}${path}`, {
        method,
        headers,
        body: body ? JSON.stringify(body) : undefined
      });
      
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(`${res.status} ${JSON.stringify(data)}`);
      return data;
    }
    
    // 加载消息列表
    async function loadMessages() {
      try {
        const result = await api(`/api/group-messages?group_id=${groupId}&limit=50`, { token });
        const container = document.getElementById('messagesContainer');
        container.innerHTML = '';
        
        result.data.messages.forEach(msg => {
          const div = document.createElement('div');
          div.className = `message ${msg.is_recalled ? 'recalled' : ''}`;
          div.innerHTML = `
            <div class="message-header">
              <img class="avatar" src="${msg.sender_avatar_url || '/default-avatar.png'}" alt="">
              <span class="sender-name">${msg.sender_nickname}</span>
              <span class="send-time">${new Date(msg.send_time).toLocaleString()}</span>
            </div>
            <div class="message-content">${msg.message_content}</div>
          `;
          container.appendChild(div);
        });
        
        // 滚动到底部
        container.scrollTop = container.scrollHeight;
      } catch (error) {
        console.error('加载消息失败:', error);
      }
    }
    
    // 发送消息
    async function sendMessage() {
      const input = document.getElementById('messageInput');
      const content = input.value.trim();
      
      if (!content) return;
      
      try {
        await api('/api/group-messages', {
          method: 'POST',
          token,
          body: {
            group_id: groupId,
            message_content: content,
            message_type: 'text'
          }
        });
        
        input.value = '';
        loadMessages(); // 刷新消息列表
      } catch (error) {
        alert('发送失败: ' + error.message);
      }
    }
    
    // 撤回消息
    async function recallMessage(messageUuid) {
      try {
        await api('/api/group-messages/recall', {
          method: 'POST',
          token,
          body: { message_uuid: messageUuid }
        });
        loadMessages();
      } catch (error) {
        alert('撤回失败: ' + error.message);
      }
    }
    
    // 删除消息（个人）
    async function deleteMessage(messageUuid) {
      try {
        await api('/api/group-messages/delete', {
          method: 'DELETE',
          token,
          body: { message_uuid: messageUuid }
        });
        loadMessages();
      } catch (error) {
        alert('删除失败: ' + error.message);
      }
    }
    
    // 回车发送消息
    document.getElementById('messageInput').addEventListener('keypress', (e) => {
      if (e.key === 'Enter') sendMessage();
    });
    
    // 初始加载
    loadMessages();
    
    // 定时刷新（简单实现，实际应使用 WebSocket）
    setInterval(loadMessages, 5000);
  </script>
</body>
</html>
```

---

## 🔐 权限控制

| 操作 | 群主 | 管理员 | 普通成员 |
|------|-----|-------|---------|
| 发送消息 | ✅ | ✅ | ✅（未禁言） |
| 获取消息 | ✅ | ✅ | ✅ |
| 删除消息（个人） | ✅ | ✅ | ✅ |
| 撤回自己的消息（2分钟内） | ✅ | ✅ | ✅ |
| 撤回任意消息 | ✅ | ✅ | ❌ |

---

## ⚠️ 注意事项

1. **消息内容不能为空**：空消息会被拒绝
2. **撤回时限**：普通成员只能撤回 2 分钟内发送的消息
3. **删除 vs 撤回**：
   - 删除：仅对自己不可见，其他人仍可看到
   - 撤回：所有人都看不到原内容
4. **禁言检查**：被禁言的用户无法发送消息
5. **分页加载**：使用 `before_time` 时间戳分页
6. **实时消息**：已支持 WebSocket 实时消息推送
7. **消息归档**：30天前的历史消息会自动归档，仍可查询
8. **发送者信息**：消息响应中包含 `sender_nickname` 和 `sender_avatar_url`，无需额外查询

