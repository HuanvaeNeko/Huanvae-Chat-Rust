# Groups 群聊接口（Fetch API）

群聊管理相关接口的前端调用示例，使用 Fetch API。

## 🔧 通用工具函数

```js
const BASE = 'http://localhost:8080';

/**
 * 通用 API 请求封装
 * @param {string} path - API 路径
 * @param {object} options - 请求选项
 */
async function api(path, { method='GET', token, body, formData }={}) {
  const headers = {};
  
  if (body) {
    headers['Content-Type'] = 'application/json';
  }
  
  if (token) {
    headers['Authorization'] = `Bearer ${token}`;
  }
  
  const options = {
    method,
    headers,
    body: formData || (body ? JSON.stringify(body) : undefined)
  };
  
  const res = await fetch(`${BASE}${path}`, options);
  const data = await res.json().catch(() => ({}));
  
  if (!res.ok) {
    throw new Error(`${res.status} ${JSON.stringify(data)}`);
  }
  
  return data;
}
```

---

## 📋 接口列表

### 一、群聊基础操作

#### 1. 创建群聊

**端点**：`POST /api/groups`

**功能**：创建一个新的群聊，创建者自动成为群主

**请求体**：
```typescript
{
  group_name: string,           // 群名称（必填）
  group_description?: string,   // 群简介（可选）
  join_mode?: string           // 入群模式（可选，默认 approval_required）
}
```

**入群模式选项**：
| 值 | 说明 |
|---|---|
| `open` | 开放入群：所有方式均可直接进入 |
| `approval_required` | 需审核：普通成员邀请和搜索入群需审核（默认） |
| `invite_only` | 仅邀请：只能通过邀请进入 |
| `admin_invite_only` | 仅管理邀请：只能群主/管理员邀请进入 |
| `closed` | 关闭入群：不允许任何新成员加入 |

**请求示例**：
```js
const result = await api('/api/groups', {
  method: 'POST',
  token,
  body: {
    group_name: '我的群聊',
    group_description: '这是一个测试群',
    join_mode: 'approval_required'
  }
});

console.log(result);
// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "group_id": "019ae4ec-0dfe-7ac1-966e-876e9755561c",
//     "group_name": "我的群聊",
//     "created_at": "2025-12-03T15:54:26.686987Z"
//   }
// }
```

---

#### 2. 获取我的群聊列表

**端点**：`GET /api/groups/my`

**功能**：获取当前用户加入的所有群聊列表

**请求示例**：
```js
const myGroups = await api('/api/groups/my', { token });

console.log(myGroups);
// {
//   "success": true,
//   "code": 200,
//   "data": [
//     {
//       "group_id": "019ae4ec-0dfe-7ac1-966e-876e9755561c",
//       "group_name": "测试群聊",
//       "group_avatar_url": "",
//       "role": "owner",
//       "unread_count": 5,
//       "last_message_content": "大家好",
//       "last_message_time": "2025-12-03T15:54:26.686987Z"
//     }
//   ]
// }
```

**响应字段说明**：
| 字段 | 类型 | 说明 |
|------|------|------|
| `group_id` | UUID | 群聊唯一标识 |
| `group_name` | string | 群名称 |
| `group_avatar_url` | string | 群头像URL |
| `role` | string | 用户在群中的角色（owner/admin/member） |
| `unread_count` | number \| null | 未读消息数量 |
| `last_message_content` | string \| null | 最后一条消息内容 |
| `last_message_time` | string \| null | 最后一条消息时间 |

---

#### 3. 获取群聊详情

**端点**：`GET /api/groups/{group_id}`

**功能**：获取指定群聊的详细信息

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
const groupInfo = await api(`/api/groups/${groupId}`, { token });

console.log(groupInfo);
// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "group_id": "019ae4ec-0dfe-7ac1-966e-876e9755561c",
//     "group_name": "测试群聊",
//     "group_avatar_url": "",
//     "group_description": "这是一个测试群",
//     "creator_id": "user_a",
//     "created_at": "2025-12-03T15:54:26.686987Z",
//     "join_mode": "approval_required",
//     "status": "active",
//     "member_count": 5
//   }
// }
```

---

#### 4. 更新群聊信息

**端点**：`PUT /api/groups/{group_id}`

**权限**：群主或管理员

**请求体**：
```typescript
{
  group_name?: string,         // 新群名称
  group_description?: string,  // 新群简介
  group_avatar_url?: string    // 新头像URL
}
```

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
await api(`/api/groups/${groupId}`, {
  method: 'PUT',
  token,
  body: {
    group_name: '新群名',
    group_description: '更新后的群简介'
  }
});
```

---

#### 5. 修改入群模式

**端点**：`PUT /api/groups/{group_id}/join-mode`

**权限**：仅群主

**请求体**：
```typescript
{
  join_mode: string  // 新的入群模式
}
```

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
await api(`/api/groups/${groupId}/join-mode`, {
  method: 'PUT',
  token,
  body: {
    join_mode: 'invite_only'
  }
});
```

---

#### 6. 解散群聊

**端点**：`DELETE /api/groups/{group_id}`

**权限**：仅群主

**功能**：软删除群聊，保留所有记录

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
await api(`/api/groups/${groupId}`, {
  method: 'DELETE',
  token
});

// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "success": true,
//     "message": "群聊已解散"
//   }
// }
```

---

### 二、成员管理

#### 1. 获取成员列表

**端点**：`GET /api/groups/{group_id}/members`

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
const members = await api(`/api/groups/${groupId}/members`, { token });

console.log(members);
// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "members": [
//       {
//         "user_id": "user_a",
//         "user_nickname": "用户A",
//         "user_avatar_url": "",
//         "role": "owner",
//         "group_nickname": null,
//         "joined_at": "2025-12-03T15:54:26.686987Z",
//         "join_method": "create",
//         "muted_until": null
//       }
//     ],
//     "total": 1
//   }
// }
```

---

#### 2. 邀请成员入群

**端点**：`POST /api/groups/{group_id}/invite`

**权限**：
- 群主/管理员：被邀请者直接加入
- 普通成员：被邀请者需要审核

**请求体**：
```typescript
{
  user_ids: string[],   // 被邀请用户ID列表
  message?: string      // 邀请消息（可选）
}
```

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
const result = await api(`/api/groups/${groupId}/invite`, {
  method: 'POST',
  token,
  body: {
    user_ids: ['user_b', 'user_c'],
    message: '欢迎加入我们的群聊'
  }
});

console.log(result);
// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "results": [
//       {
//         "user_id": "user_b",
//         "success": true,
//         "message": "邀请已发送，待对方同意"
//       }
//     ]
//   }
// }
```

---

#### 3. 退出群聊

**端点**：`POST /api/groups/{group_id}/leave`

**注意**：群主不能退出，需先转让群主或解散群聊

**请求体**：
```typescript
{
  reason?: string  // 退出原因（可选）
}
```

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
await api(`/api/groups/${groupId}/leave`, {
  method: 'POST',
  token,
  body: {
    reason: '个人原因'
  }
});

// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "success": true,
//     "message": "已退出群聊"
//   }
// }
```

---

#### 4. 移除成员

**端点**：`DELETE /api/groups/{group_id}/members/{user_id}`

**权限**：
- 群主：可移除任何成员
- 管理员：只能移除普通成员

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
const userId = 'user_b';
await api(`/api/groups/${groupId}/members/${userId}`, {
  method: 'DELETE',
  token
});

// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "success": true,
//     "message": "成员已移除"
//   }
// }
```

---

### 三、角色管理

#### 1. 转让群主

**端点**：`POST /api/groups/{group_id}/transfer`

**权限**：仅群主

**功能**：
- 新群主从管理员/普通成员变为群主
- 旧群主变为普通成员

**请求体**：
```typescript
{
  new_owner_id: string  // 新群主用户ID
}
```

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
await api(`/api/groups/${groupId}/transfer`, {
  method: 'POST',
  token,
  body: {
    new_owner_id: 'user_b'
  }
});

// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "success": true,
//     "message": "群主已转让"
//   }
// }
```

---

#### 2. 设置管理员

**端点**：`POST /api/groups/{group_id}/admins`

**权限**：仅群主

**请求体**：
```typescript
{
  user_id: string  // 要设置为管理员的用户ID
}
```

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
await api(`/api/groups/${groupId}/admins`, {
  method: 'POST',
  token,
  body: {
    user_id: 'user_b'
  }
});

// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "success": true,
//     "message": "已设置为管理员"
//   }
// }
```

---

#### 3. 取消管理员

**端点**：`DELETE /api/groups/{group_id}/admins/{user_id}`

**权限**：仅群主

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
const userId = 'user_b';
await api(`/api/groups/${groupId}/admins/${userId}`, {
  method: 'DELETE',
  token
});

// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "success": true,
//     "message": "已取消管理员"
//   }
// }
```

---

### 四、禁言管理

#### 1. 禁言成员

**端点**：`POST /api/groups/{group_id}/mute`

**权限**：
- 群主：可禁言任何成员
- 管理员：只能禁言普通成员

**请求体**：
```typescript
{
  user_id: string,      // 要禁言的用户ID
  duration_minutes: number  // 禁言时长（分钟）
}
```

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
await api(`/api/groups/${groupId}/mute`, {
  method: 'POST',
  token,
  body: {
    user_id: 'user_b',
    duration_minutes: 60  // 禁言 1 小时
  }
});

// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "success": true,
//     "message": "已禁言",
//     "muted_until": "2025-12-03T16:54:26.686987Z"
//   }
// }
```

---

#### 2. 解除禁言

**端点**：`DELETE /api/groups/{group_id}/mute/{user_id}`

**权限**：群主或管理员

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
const userId = 'user_b';
await api(`/api/groups/${groupId}/mute/${userId}`, {
  method: 'DELETE',
  token
});

// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "success": true,
//     "message": "已解除禁言"
//   }
// }
```

---

### 五、邀请码管理

#### 1. 生成邀请码

**端点**：`POST /api/groups/{group_id}/invite-codes`

**权限**：
- 群主/管理员：生成"直通"邀请码（direct），使用者可直接入群
- 普通成员：生成"普通"邀请码（normal），使用者需审核

**请求体**：
```typescript
{
  max_uses?: number,       // 最大使用次数（可选，默认 1）
  expires_in_hours?: number  // 有效期（小时，可选，默认 24）
}
```

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
const result = await api(`/api/groups/${groupId}/invite-codes`, {
  method: 'POST',
  token,
  body: {
    max_uses: 10,
    expires_in_hours: 48
  }
});

console.log(result);
// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "id": "019ae4f0-c860-7570-842b-348c48552a1b",
//     "code": "wD14KR1K",
//     "code_type": "direct",
//     "expires_at": "2025-12-05T15:59:36.544750Z"
//   }
// }
```

---

#### 2. 获取邀请码列表

**端点**：`GET /api/groups/{group_id}/invite-codes`

**权限**：群主或管理员

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
const codes = await api(`/api/groups/${groupId}/invite-codes`, { token });
```

---

#### 3. 撤销邀请码

**端点**：`DELETE /api/groups/{group_id}/invite-codes/{code_id}`

**权限**：邀请码创建者或群主/管理员

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
const codeId = '019ae4f0-c860-7570-842b-348c48552a1b';
await api(`/api/groups/${groupId}/invite-codes/${codeId}`, {
  method: 'DELETE',
  token
});
```

---

#### 4. 通过邀请码入群

**端点**：`POST /api/groups/join-by-code`

**请求体**：
```typescript
{
  code: string  // 邀请码
}
```

**请求示例**：
```js
const result = await api('/api/groups/join-by-code', {
  method: 'POST',
  token,
  body: {
    code: 'wD14KR1K'
  }
});

console.log(result);
// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "success": true,
//     "message": "已成功加入群聊"
//   }
// }
```

---

### 六、入群申请

#### 1. 申请入群（搜索方式）

**端点**：`POST /api/groups/{group_id}/apply`

**请求体**：
```typescript
{
  message?: string  // 申请消息（可选）
}
```

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
const result = await api(`/api/groups/${groupId}/apply`, {
  method: 'POST',
  token,
  body: {
    message: '你好，我想加入这个群'
  }
});
```

---

#### 2. 获取收到的邀请

**端点**：`GET /api/groups/invitations`

**功能**：获取用户收到的所有群聊邀请

**请求示例**：
```js
const invitations = await api('/api/groups/invitations', { token });

console.log(invitations);
// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "invitations": [
//       {
//         "request_id": "019ae4ec-7a9e-77a3-82f4-98ca71fc97de",
//         "group_id": "019ae4ec-0dfe-7ac1-966e-876e9755561c",
//         "group_name": "测试群聊",
//         "group_avatar_url": "",
//         "inviter_id": "user_a",
//         "inviter_nickname": "用户A",
//         "message": "欢迎加入",
//         "created_at": "2025-12-03T15:54:54.494997Z",
//         "expires_at": "2025-12-10T15:54:54.494997Z"
//       }
//     ]
//   }
// }
```

---

#### 3. 接受邀请

**端点**：`POST /api/groups/invitations/{request_id}/accept`

**请求示例**：
```js
const requestId = '019ae4ec-7a9e-77a3-82f4-98ca71fc97de';
await api(`/api/groups/invitations/${requestId}/accept`, {
  method: 'POST',
  token
});

// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "success": true,
//     "message": "已成功加入群聊"
//   }
// }
```

---

#### 4. 拒绝邀请

**端点**：`POST /api/groups/invitations/{request_id}/decline`

**请求示例**：
```js
const requestId = '019ae4ec-7a9e-77a3-82f4-98ca71fc97de';
await api(`/api/groups/invitations/${requestId}/decline`, {
  method: 'POST',
  token
});
```

---

#### 5. 获取待处理申请（管理员）

**端点**：`GET /api/groups/{group_id}/requests`

**权限**：群主或管理员

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
const requests = await api(`/api/groups/${groupId}/requests`, { token });
```

---

#### 6. 同意申请

**端点**：`POST /api/groups/{group_id}/requests/{request_id}/approve`

**权限**：群主或管理员

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
const requestId = '019ae4ec-xxxx';
await api(`/api/groups/${groupId}/requests/${requestId}/approve`, {
  method: 'POST',
  token
});
```

---

#### 7. 拒绝申请

**端点**：`POST /api/groups/{group_id}/requests/{request_id}/reject`

**权限**：群主或管理员

**请求体**：
```typescript
{
  reason?: string  // 拒绝原因（可选）
}
```

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
const requestId = '019ae4ec-xxxx';
await api(`/api/groups/${groupId}/requests/${requestId}/reject`, {
  method: 'POST',
  token,
  body: {
    reason: '不符合入群条件'
  }
});
```

---

### 七、群公告

#### 1. 发布公告

**端点**：`POST /api/groups/{group_id}/notices`

**权限**：群主或管理员

**请求体**：
```typescript
{
  title: string,      // 公告标题
  content: string,    // 公告内容
  is_pinned?: boolean // 是否置顶（可选，默认 false）
}
```

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
const result = await api(`/api/groups/${groupId}/notices`, {
  method: 'POST',
  token,
  body: {
    title: '欢迎新成员',
    content: '欢迎大家加入群聊！',
    is_pinned: true
  }
});

console.log(result);
// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "id": "019ae4ed-38de-7bf2-b408-19033e450ba9",
//     "published_at": "2025-12-03T15:55:43.198074Z"
//   }
// }
```

---

#### 2. 获取公告列表

**端点**：`GET /api/groups/{group_id}/notices`

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
const notices = await api(`/api/groups/${groupId}/notices`, { token });

console.log(notices);
// {
//   "success": true,
//   "code": 200,
//   "data": {
//     "notices": [
//       {
//         "id": "019ae4ed-38de-7bf2-b408-19033e450ba9",
//         "title": "欢迎新成员",
//         "content": "欢迎大家加入群聊！",
//         "publisher_id": "user_a",
//         "publisher_nickname": "用户A",
//         "published_at": "2025-12-03T15:55:43.198074Z",
//         "is_pinned": true,
//         "updated_at": "2025-12-03T15:55:43.198610Z"
//       }
//     ]
//   }
// }
```

---

#### 3. 更新公告

**端点**：`PUT /api/groups/{group_id}/notices/{notice_id}`

**权限**：群主或管理员

**请求体**：
```typescript
{
  title?: string,      // 新标题
  content?: string,    // 新内容
  is_pinned?: boolean  // 是否置顶
}
```

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
const noticeId = '019ae4ed-38de-7bf2-b408-19033e450ba9';
await api(`/api/groups/${groupId}/notices/${noticeId}`, {
  method: 'PUT',
  token,
  body: {
    title: '更新后的标题',
    is_pinned: false
  }
});
```

---

#### 4. 删除公告

**端点**：`DELETE /api/groups/{group_id}/notices/{notice_id}`

**权限**：群主或管理员

**请求示例**：
```js
const groupId = '019ae4ec-0dfe-7ac1-966e-876e9755561c';
const noticeId = '019ae4ed-38de-7bf2-b408-19033e450ba9';
await api(`/api/groups/${groupId}/notices/${noticeId}`, {
  method: 'DELETE',
  token
});
```

---

## 🔐 权限控制总结

| 操作 | 群主 | 管理员 | 普通成员 |
|------|-----|-------|---------|
| 修改群信息 | ✅ | ✅ | ❌ |
| 修改入群模式 | ✅ | ❌ | ❌ |
| 邀请成员（直接） | ✅ | ✅ | ❌ |
| 邀请成员（需审核） | - | - | ✅ |
| 生成直通邀请码 | ✅ | ✅ | ❌ |
| 生成普通邀请码 | ✅ | ✅ | ✅ |
| 审核入群申请 | ✅ | ✅ | ❌ |
| 设置管理员 | ✅ | ❌ | ❌ |
| 移除成员 | ✅ | ✅（不能移除管理员） | ❌ |
| 禁言成员 | ✅ | ✅（不能禁言管理员） | ❌ |
| 发布公告 | ✅ | ✅ | ❌ |
| 转让群主 | ✅ | ❌ | ❌ |
| 解散群聊 | ✅ | ❌ | ❌ |

---

## ⚠️ 注意事项

1. **群主限制**：群主不能退出群聊，需先转让群主或解散群聊
2. **软删除**：解散群聊是软删除，保留所有历史记录
3. **邀请码类型**：
   - `direct`：群主/管理员生成，使用者可直接入群
   - `normal`：普通成员生成，使用者需要审核
4. **入群方式记录**：系统会记录每个成员的入群方式
5. **转让群主**：新群主如果是管理员，会从管理员变为群主；旧群主变为普通成员

