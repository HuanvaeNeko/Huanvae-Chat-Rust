# Profile 接口（Fetch API）

个人资料管理相关接口的前端调用示例，使用 Fetch API。

## 🔧 通用工具函数

```js
const BASE = 'http://localhost:8080';

/**
 * 通用 API 请求封装
 * @param {string} path - API 路径
 * @param {object} options - 请求选项
 * @param {string} options.method - HTTP 方法
 * @param {string} options.token - Bearer Token
 * @param {object} options.body - 请求体（自动转 JSON）
 * @param {FormData} options.formData - FormData 对象（用于文件上传）
 */
async function api(path, { method='GET', token, body, formData }={}) {
  const headers = {};
  
  // JSON 请求
  if (body) {
    headers['Content-Type'] = 'application/json';
  }
  
  // 认证 Token
  if (token) {
    headers['Authorization'] = `Bearer ${token}`;
  }
  
  const options = {
    method,
    headers,
    body: formData || (body ? JSON.stringify(body) : undefined)
  };
  
  const res = await fetch(`${BASE}${path}`, options);
  const data = await res.json().catch(() => ({}));
  
  if (!res.ok) {
    throw new Error(`${res.status} ${JSON.stringify(data)}`);
  }
  
  return data;
}
```

---

## 📋 接口列表

### 1. 获取个人信息（受保护）

**端点**：`GET /api/profile`

**功能**：获取当前用户的完整个人信息（不含密码）

**鉴权**：需要 Bearer Token

**请求示例**：
```js
// 获取个人信息
const profile = await api('/api/profile', { token });

console.log(profile);
// 输出:
// {
//   "data": {
//     "user_id": "testuser001",
//     "user_nickname": "测试用户",
//     "user_email": "test@example.com",
//     "user_signature": "Hello, world!",
//     "user_avatar_url": "http://localhost:9000/avatars/testuser001.jpg",
//     "admin": "false",
//     "created_at": "2025-11-25T11:00:02.221791Z",
//     "updated_at": "2025-11-25T11:03:40.730806Z"
//   }
// }
```

**响应字段说明**：

| 字段 | 类型 | 说明 |
|------|------|------|
| `user_id` | string | 用户 ID |
| `user_nickname` | string | 昵称 |
| `user_email` | string\|null | 邮箱 |
| `user_signature` | string\|null | 个性签名 |
| `user_avatar_url` | string\|null | 头像 URL |
| `admin` | string | 是否管理员 ("true"/"false") |
| `created_at` | string | 创建时间（ISO 8601） |
| `updated_at` | string | 更新时间（ISO 8601） |

---

### 2. 更新个人信息（受保护）

**端点**：`PUT /api/profile`

**功能**：更新用户的邮箱和/或个性签名

**鉴权**：需要 Bearer Token

**请求体**：
```typescript
{
  email?: string,        // 新邮箱（可选）
  signature?: string     // 个性签名（可选）
}
```

**验证规则**：
- `email`: 必须是有效邮箱格式
- `signature`: 最长 200 字符
- 至少提供一个字段

**请求示例**：
```js
// 仅更新邮箱
await api('/api/profile', {
  method: 'PUT',
  token,
  body: {
    email: 'newemail@example.com'
  }
});

// 仅更新签名
await api('/api/profile', {
  method: 'PUT',
  token,
  body: {
    signature: 'This is my new signature!'
  }
});

// 同时更新邮箱和签名
await api('/api/profile', {
  method: 'PUT',
  token,
  body: {
    email: 'newemail@example.com',
    signature: 'Hello, world!'
  }
});
```

**成功响应**：
```json
{
  "message": "Profile updated successfully"
}
```

**错误响应**：
```json
{
  "error": "Validation error: email: Invalid email format"
}
```

---

### 3. 修改密码（受保护）

**端点**：`PUT /api/profile/password`

**功能**：修改用户密码

**鉴权**：需要 Bearer Token

**请求体**：
```typescript
{
  old_password: string,  // 旧密码（必填）
  new_password: string   // 新密码（必填）
}
```

**验证规则**：
- `old_password`: 至少 6 字符
- `new_password`: 6-100 字符

**请求示例**：
```js
await api('/api/profile/password', {
  method: 'PUT',
  token,
  body: {
    old_password: 'oldpass123',
    new_password: 'newpass456'
  }
});
```

**成功响应**：
```json
{
  "message": "Password updated successfully"
}
```

**错误响应**：

旧密码错误（401）：
```json
{
  "error": "Old password is incorrect"
}
```

验证错误（400）：
```json
{
  "error": "Validation error: new_password: Password must be 6-100 characters"
}
```

**重要提示**：
- 密码修改后立即生效
- 旧密码验证失败返回 401 状态码
- 建议使用 HTTPS 传输密码

---

### 4. 上传头像（受保护）

**端点**：`POST /api/profile/avatar`

**功能**：上传用户头像

**鉴权**：需要 Bearer Token

**请求格式**：`multipart/form-data`

**表单字段**：
- `avatar` 或 `file`: 图片文件

**支持格式**：jpg, jpeg, png, gif, webp

**大小限制**：最大 10MB

**请求示例**：

```js
// 方法 1：从文件输入上传
const fileInput = document.querySelector('#avatarInput');
const file = fileInput.files[0];

const formData = new FormData();
formData.append('avatar', file);

const result = await api('/api/profile/avatar', {
  method: 'POST',
  token,
  formData
});

console.log(result);
// {
//   "avatar_url": "http://localhost:9000/avatars/testuser001.jpg",
//   "message": "Avatar uploaded successfully"
// }
```

```js
// 方法 2：从 Blob 上传
const blob = await fetch('https://example.com/image.jpg').then(r => r.blob());

const formData = new FormData();
formData.append('avatar', blob, 'avatar.jpg');

const result = await api('/api/profile/avatar', {
  method: 'POST',
  token,
  formData
});
```

```js
// 方法 3：完整示例（带错误处理）
async function uploadAvatar(file, token) {
  try {
    // 验证文件大小
    const maxSize = 10 * 1024 * 1024; // 10MB
    if (file.size > maxSize) {
      alert('文件太大，最大 10MB');
      return;
    }
    
    // 验证文件类型
    const allowedTypes = ['image/jpeg', 'image/png', 'image/gif', 'image/webp'];
    if (!allowedTypes.includes(file.type)) {
      alert('不支持的文件格式');
      return;
    }
    
    // 上传
    const formData = new FormData();
    formData.append('avatar', file);
    
    const result = await api('/api/profile/avatar', {
      method: 'POST',
      token,
      formData
    });
    
    // 更新页面上的头像显示
    document.querySelector('#userAvatar').src = result.avatar_url;
    alert('头像上传成功！');
    
    return result.avatar_url;
  } catch (error) {
    console.error('上传失败:', error);
    alert('上传失败: ' + error.message);
  }
}

// 使用示例
document.querySelector('#avatarInput').addEventListener('change', async (e) => {
  const file = e.target.files[0];
  if (file) {
    await uploadAvatar(file, token);
  }
});
```

**成功响应**：
```json
{
  "avatar_url": "http://localhost:9000/avatars/testuser001.jpg",
  "message": "Avatar uploaded successfully"
}
```

**错误响应**：

无文件上传（400）：
```json
{
  "error": "No file uploaded. Use field name 'avatar' or 'file'"
}
```

文件太大（400）：
```json
{
  "error": "File too large. Maximum size: 10 MB, got: 12.42 MB"
}
```

不支持的格式（400）：
```json
{
  "error": "Unsupported file format. Allowed: jpg, jpeg, png, gif, webp"
}
```

---

## 🎨 完整示例：个人资料页面

```html
<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
  <title>个人资料</title>
  <style>
    .profile-container { max-width: 600px; margin: 50px auto; padding: 20px; }
    .avatar { width: 150px; height: 150px; border-radius: 50%; }
    .form-group { margin-bottom: 15px; }
    label { display: block; margin-bottom: 5px; font-weight: bold; }
    input, textarea { width: 100%; padding: 8px; box-sizing: border-box; }
    button { padding: 10px 20px; background: #007bff; color: white; border: none; cursor: pointer; }
    button:hover { background: #0056b3; }
  </style>
</head>
<body>
  <div class="profile-container">
    <h1>个人资料</h1>
    
    <!-- 头像显示 -->
    <div class="form-group">
      <img id="userAvatar" class="avatar" src="/default-avatar.png" alt="头像">
      <input type="file" id="avatarInput" accept="image/*">
      <button onclick="handleAvatarUpload()">上传头像</button>
    </div>
    
    <!-- 基本信息 -->
    <div class="form-group">
      <label>用户 ID</label>
      <input id="userId" type="text" disabled>
    </div>
    
    <div class="form-group">
      <label>昵称</label>
      <input id="nickname" type="text" disabled>
    </div>
    
    <div class="form-group">
      <label>邮箱</label>
      <input id="email" type="email">
    </div>
    
    <div class="form-group">
      <label>个性签名</label>
      <textarea id="signature" rows="3"></textarea>
    </div>
    
    <button onclick="handleUpdateProfile()">保存修改</button>
    
    <!-- 修改密码 -->
    <h2>修改密码</h2>
    <div class="form-group">
      <label>旧密码</label>
      <input id="oldPassword" type="password">
    </div>
    
    <div class="form-group">
      <label>新密码</label>
      <input id="newPassword" type="password">
    </div>
    
    <button onclick="handleUpdatePassword()">修改密码</button>
  </div>

  <script>
    const BASE = 'http://localhost:8080';
    let token = localStorage.getItem('access_token'); // 从 localStorage 获取 token
    
    // API 请求封装
    async function api(path, { method='GET', token, body, formData }={}) {
      const headers = {};
      if (body) headers['Content-Type'] = 'application/json';
      if (token) headers['Authorization'] = `Bearer ${token}`;
      
      const res = await fetch(`${BASE}${path}`, {
        method,
        headers,
        body: formData || (body ? JSON.stringify(body) : undefined)
      });
      
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(`${res.status} ${JSON.stringify(data)}`);
      return data;
    }
    
    // 加载个人信息
    async function loadProfile() {
      try {
        const response = await api('/api/profile', { token });
        const profile = response.data;
        
        document.querySelector('#userId').value = profile.user_id;
        document.querySelector('#nickname').value = profile.user_nickname;
        document.querySelector('#email').value = profile.user_email || '';
        document.querySelector('#signature').value = profile.user_signature || '';
        
        if (profile.user_avatar_url) {
          document.querySelector('#userAvatar').src = profile.user_avatar_url;
        }
      } catch (error) {
        alert('加载失败: ' + error.message);
      }
    }
    
    // 更新个人信息
    async function handleUpdateProfile() {
      try {
        const email = document.querySelector('#email').value;
        const signature = document.querySelector('#signature').value;
        
        await api('/api/profile', {
          method: 'PUT',
          token,
          body: { email, signature }
        });
        
        alert('保存成功！');
      } catch (error) {
        alert('保存失败: ' + error.message);
      }
    }
    
    // 修改密码
    async function handleUpdatePassword() {
      try {
        const old_password = document.querySelector('#oldPassword').value;
        const new_password = document.querySelector('#newPassword').value;
        
        await api('/api/profile/password', {
          method: 'PUT',
          token,
          body: { old_password, new_password }
        });
        
        alert('密码修改成功！');
        document.querySelector('#oldPassword').value = '';
        document.querySelector('#newPassword').value = '';
      } catch (error) {
        alert('修改失败: ' + error.message);
      }
    }
    
    // 上传头像
    async function handleAvatarUpload() {
      const file = document.querySelector('#avatarInput').files[0];
      if (!file) {
        alert('请选择文件');
        return;
      }
      
      try {
        const formData = new FormData();
        formData.append('avatar', file);
        
        const result = await api('/api/profile/avatar', {
          method: 'POST',
          token,
          formData
        });
        
        document.querySelector('#userAvatar').src = result.avatar_url;
        alert('头像上传成功！');
      } catch (error) {
        alert('上传失败: ' + error.message);
      }
    }
    
    // 页面加载时获取个人信息
    loadProfile();
  </script>
</body>
</html>
```

---

## 🔐 Token 管理建议

```js
// 存储 Token
function saveToken(accessToken, refreshToken) {
  localStorage.setItem('access_token', accessToken);
  localStorage.setItem('refresh_token', refreshToken);
}

// 获取 Token
function getToken() {
  return localStorage.getItem('access_token');
}

// 清除 Token
function clearTokens() {
  localStorage.removeItem('access_token');
  localStorage.removeItem('refresh_token');
}

// 检查 Token 是否存在
function hasToken() {
  return !!localStorage.getItem('access_token');
}
```

---

## ⚠️ 注意事项

1. **HTTPS**：生产环境必须使用 HTTPS
2. **Token 安全**：不要在 URL 中传递 Token
3. **文件大小**：客户端应在上传前验证文件大小
4. **错误处理**：始终处理 API 调用的错误情况
5. **用户体验**：上传大文件时显示进度条
6. **刷新显示**：更新成功后刷新页面显示的信息

